...
export const RazorpayPaymentButton = ({ session, placeOrder, shippingmethods, notReady, cart }) => {
  ...
  useEffect(() => {
    setOrderData(session?.data);
  }, [session?.data]);

  console.log(" this seessioon form rozarpay " , session)
  ...
  const handlePayment = useCallback(async () => {
    const options = {
      ...
      name: process.env.COMPANY_NAME ?? "Majestic peacock ",
  ...
  return (
    <>
      {loading && (
        <div className="fixed bg-gray-400/10 w-screen h-screen top-0 z-[99999] left-0 backdrop-blur-md ">
          <div className="flex justify-center items-center h-full">
            ...
            Please Select a Shipping method before place order don't panic your transaction is safe 
              <Shipping
                availableShippingMethods={shippingmethods}
                onPayment={placeOrder}
              />
      ...
      <Button
        disabled={submitting || !orderData?.id || orderData.id == ""}
        onClick={() => {
          handlePayment();
        }}
        className="w-1/2 max-w-[300px] ml-32 py-5 h-fit text-white font-semibold bg-blue-600"
      >
        {submitting}
        {submitting ? (
          <Spinner />
